	...
	<body>
		...
		<div id="show-location" class="content scaffold-show" role="main">
			...
			<ol class="property-list location">
				...
				<g:if test="${locationInstance?.contactInfo}">
				<li class="fieldcontain">
					<span id="contactInfo-label" class="property-label"><g:message code="location.contactInfo.label" default="Contact Info" /></span>
					
						<span class="property-value" aria-labelledby="contactInfo-label"><g:link controller="contactInfo" action="show" id="${locationInstance?.contactInfo?.id}">${locationInstance?.contactInfo?.encodeAsHTML()}</g:link></span>
